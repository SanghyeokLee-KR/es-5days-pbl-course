...
- 이 주제를 선택한 이유: 메시지 검색에 서비스랑 레벨 조건, 처리시간 범위까지 같이 걸어보고 싶어서
...
- 문서 수: 1,000건
- 데이터 생성 규칙과 seed: seed `20260901`, log_level은 70/20/10을 목표로 잡았고 실제로 71.1/18.8/10.1이 나왔다
- 개인정보 미사용 확인: 전부 합성 로그라 개인정보 없음
- 핵심 필드와 타입 선택 이유: log_level, service_name, exception_class는 걸러내고 세야 해서 keyword. message는 문장을 찾아야 해서 text. duration_ms, http_status는 범위로 걸러야 해서 숫자
...
- 데이터 생성 설정파일 인코딩(BOM 누락) 때문에 검색이 1건만 잡히던 걸 찾아서 고쳤다
...
- control/filter 목적: 시간이 모자라서 안 만들었다 ([evidence/day-04/dashboard-review.md](evidence/day-04/dashboard-review.md))

## 6. AI Search 확장 판단

- 적용 여부와 근거: Day 5 전이라 아직 안 정했다
