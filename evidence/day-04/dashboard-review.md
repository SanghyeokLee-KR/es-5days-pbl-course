...
filter/control 미생성. 서비스별로 나눠 보는 건 못 함.
...
| Treemap ERROR 소계 11.21% | Donut의 ERROR | 10.1% | 조금 다름 | 예외로 한 번 더 쪼갠 하위 집계. Inspect 미확인 |

## 4. 결과 해석

조건 → 핵심값 → 비교 → 판단/다음 행동 순서로 2문장 이상 작성합니다.

1. log_level INFO 71.1%, WARN 18.8%, ERROR 10.1%. 목표 70/20/10이랑 붙어서 데이터는 쓸 만함.
2. ERROR 안에서 NullPointerException 6건 > SocketTimeoutException 4건. null 참조부터 볼 것.
...
- 내 Dashboard에서 단정할 수 없는 것: 특정 서비스에 예외가 몰리는지. control이 없어서 못 나눔

## 6. 개선 전·후

- 발견한 문제: Donut/Bar 중복, http_status 패널 2개 중복(Lens 기본 제목), control 없음
- 개선 전 설정 또는 화면: `evidence/day-04/personal-dashboard.png`
- 수정한 내용: 없음
- 수정한 이유: 패널 수가 기준(4개 이상)을 넘어서 현 상태 유지
- 개선 후 확인 결과: 해당 없음

## 7. 최종 제출 체크

- [x] 모든 패널 제목이 질문과 연결된다. (http_status 2개는 Lens 기본 라벨)
- [x] 라벨·숫자·축이 겹치거나 잘리지 않는다.
- [x] 의도하지 않은 KQL·filter pill이 남아 있지 않다.
- [ ] filter/control이 관련 패널에 함께 적용된다. (미생성)
