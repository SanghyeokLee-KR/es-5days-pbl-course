...
- 실제 Metric 값: 9,818 (Last 1 year 기준). 전체는 10,000(`GET /products/_count`)
- 시간 범위: Last 1 year
- KQL/filter/control 상태: 없음
- 정상/보류/오류와 이유: YELLOW. (1) 데이터셋 버전 차이 10,000 vs 20,000, (2) 182건이 `Last 1 year` 밖
...
- Number of values: 8
- 표시된 category 수: 8
- 각 category 값이 공통 기준과 일치하는가: 8개 모두 1,250건씩, 합계 10,000(`GET /products/_search` terms). 가이드 8개×2,500=20,000과 총량만 다르고 균등 분포는 동일
- 캡처 파일: `evidence/day-04/common-dashboard.png`
...
- 최종 선택: vertical(저장된 상태)
- 선택 이유: 미수행. 처음 만든 vertical 그대로 둠
...
미수행. category Control을 안 만들어서 이 상황이 안 생긴다(`evidence/day-04/dashboard-review.md` 2절).
...
- 전체 규모가 의미하는 것: 쌓인 로그 이벤트 전체 건수
- 범주 field: `log_level`
- 실제 고유값 수: 3 (`INFO`, `WARN`, `ERROR`)
- Top N 선택값과 이유: 3. 고유값이 3개뿐이라 전부 표시
- 예상 사용자 판단: 어떤 레벨이 가장 흔한지, ERROR 비중이 갑자기 늘었는지
- 실제 제작 여부: 제작 완료. `전체 로그 수` Metric(1,000), `log_level` Donut/Bar
- 부족한 경우 필요한 field와 예시값: 해당 없음
...
**판정: YELLOW**. 필수 패널 2개(Metric, category Bar) 제작·저장 완료, 문제3과 문제4는 미수행.
