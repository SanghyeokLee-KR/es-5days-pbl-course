...
- `hits.total.value`와 반환 문서 수가 다를 수 있는 이유: 전체는 10000건인데 `size`가 5라서
...
- 반환 문서 수: 3
- `_source`에 요구하지 않은 field가 포함됐는가: 아니오. 지정한 4개 field만 나옴
...
- 오름차순 여부: 맞음
- 두 문서의 price가 같을 때 순서가 고정된다고 말할 수 있는가? 근거: 아니오. 5900원 문서가 4건인데 정렬 기준이 `price` 하나뿐이라 2차 기준이 없음
...
- 반환 문서 수: 5
- 판정과 근거: 통과. `_count`와 `hits.total.value` 둘 다 1000, `size: 5`는 응답 개수만 제한
...
- 포함한 field와 이유: `log_id`(식별), `service_name`(발생 서비스), `log_level`(심각도), `message`(내용), `duration_ms`(처리 시간)
- 제외한 field와 이유: `logger_name`, `trace_id`, `exception_class`, `http_status`. 상세 화면에서 봐도 되는 정보
