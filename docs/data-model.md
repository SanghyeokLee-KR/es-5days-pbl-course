...
- ES `_id`와 업무 ID 관계: Bulk 적재 시 `_id`를 `log_id` 값과 동일하게 지정한다(1:1 대응). `GET /shop-logs/_doc/LOG-000002`처럼 업무 ID로 바로 문서를 조회할 수 있다.
...
- 배열/객체 여부와 제공 생성기 지원 범위: 전부 flat scalar field다. 배열·nested 객체는 쓰지 않았다 — 생성기가 scalar와 단순 배열만 지원하고, 우리 로그 도메인에는 배열이 필요한 field가 없었다.
- 제외한 개인정보/불필요한 field와 이유: 이용자 ID·이메일·결제 수단·접속 IP는 사용하지 않는다. 장애 원인을 로그로 찾는 과제라 개인정보가 필요 없다. 요청 추적은 합성 `trace_id`로 대신한다.
- 자가 점검으로 수정한 내용: Day 1 초안의 `@timestamp`는 생성기가 field 이름에 `@`를 허용하지 않아 `timestamp`로 바꿨다. Kibana에서 Data View 시간 field로 아무 `date` field나 지정할 수 있어 Day 4 작업에는 영향이 없다. `message`도 `nori` 플러그인이 이 강의용 ES 이미지에 없어(`GET /_cat/plugins?v` 결과 없음) `standard` analyzer로 대체했다.
