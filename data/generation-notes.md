...
|---|---|---|---|---|---|
| "시간 초과" 언급 로그 검색 | message가 "게이트웨이 응답이 3000ms 후 시간 초과되었습니다."인 문서 (약 10%) | "요청 처리가 정상적으로 완료되었습니다."인 문서 (약 70%) | "시간"만 있고 "초과"가 없는 문장은 미생성 | 예 (`LOG-000002`) | Day3에서 100건 |
| 결제 서비스의 느린 ERROR | payment-api, ERROR, duration_ms 3000 초과 (`duration_ms=3021`) | 같은 조건에 duration_ms 3000 이하 | duration_ms가 딱 3000인 문서는 균등분포라 보장 안 됨 | 예 (`LOG-000002`) | Day3에서 3건 |
| 서비스별 에러 요약 | log_level ERROR 약 100건, 4개 서비스 분산 | log_level INFO 약 700건 | 서비스 하나에 ERROR 0건. 서비스당 25건쯤이라 가능성 낮음 | 아니오 | Day3에서 payment-api ERROR 20건 |

- FixedDocumentsFile을 쓰면 업무 ID를 생성 ID로 다시 배정함을 확인: 확인함. `LOG-000001`~`003`
- 복합 객체/연관 분포 등 미지원 요구와 선택한 범위/대안: field 간 조건부 상관관계 미지원. `log_level=ERROR`랑 `exception_class`, `http_status`가 따로 뽑힘. 각각 독립 확률로 두고 목표 비율만 맞춤
- 생성·적재·검증 명령: `generate-data.ps1` → `validate-data.ps1` → `load-data.ps1`
- 실제 분포와 예상의 차이 / 다음 수정: validate PASS(1000건, ID 중복 없음), 적재 후 `_count` 1000건. log_level 목표 70/20/10에 실측 71.1/18.8/10.1. duration_ms min 50.0, max 4994.0, avg 2557.98. 수정 없음
